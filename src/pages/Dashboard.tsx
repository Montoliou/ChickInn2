...
export function Dashboard() {
  ...
  const stats = {
    heute: eggs.filter(e => isSameDay(e.laidAt, now)).length,
    woche: eggs.filter(e => isSameWeek(e.laidAt, now)).length,
    monat: eggs.filter(e => isSameMonth(e.laidAt, now)).length,
    jahr:  eggs.filter(e => isSameYear(e.laidAt, now)).length,
  }
      ...
      <div className="grid grid-cols-2 gap-3">
        {Object.entries(stats).map(([label, val]) => (
          <div key={label} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 text-center">
            ...
            <p className="text-4xl font-bold text-green-600 mt-1">{val}</p>
          </div>
        ))}
      </div>
        ...
        {chickens.length === 0 ? (
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            {chickens.map(chicken => (
              ...
              >
                <div className="w-16 h-16 rounded-full bg-green-100 border-2 border-green-400 flex items-center justify-center text-2xl overflow-hidden">
                  {chicken.photoUrl
              ...
              return (
                <div key={egg.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-800">{chicken?.name ?? '—'}</p>
